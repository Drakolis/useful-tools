const { readdirSync, readFileSync, writeFileSync, appendFileSync } = require("fs");
...
const properModules = modules.filter(module => !module.startsWith(".")).reduce((accumulator, module) => {
  if(!module.includes("@")) {
    return [...accumulator, module]
  } else {
    const submodules = readdirSync(join(path, module)).map(submodule => join(module, submodule));
...
moduleData.forEach(data => {
  ...
  if(typeof authorString === "object") {
    if (!Object.keys(authorString).includes("name") && !Object.keys(authorString).includes("email") && !Object.keys(authorString).includes("url")) {
  ...
  appendFileSync(licensesMdPath, `${data.name} | ${data.version} | ${data.license} | ${authorString}
`)
})

appendFileSync(licensesMdPath, `\n`)

Object.keys(licenseData).forEach(license => {
